...
<?php $stateLang = $state == 'pv' ? 'views' : 'message';?>
  ...
  <header class='clearfix'>
    <ul class='nav nav-tabs-main pull-left'>
      ...
      <li <?php if($mode == $code) {echo "class='active'";}?>>
        <?php echo html::a(inlink('articleRank', "&mode=$code&state=$state"), $modeName);?>
      </li>
      <?php endforeach;?>
    </ul>
      ...
      <div class="dropdown" id="switchBar"><a href="###" data-toggle="dropdown" class="btn"><?php echo $lang->stat->page->{$stateLang};?> <span class="icon-angle-down"></span></a>
        <ul class="dropdown-menu">
          <li <?php echo $state == 'pv' ? "class='active'" : '';?> data-type='pv'>
            <a href="<?php echo inlink('articleRank', "mode=$mode&state=pv");?>"><?php echo $lang->stat->page->views;?></a>
          </li>
          <li <?php echo $state == 'extra' ? "class='active'" : '';?> data-type='extra'>
            <a href="<?php echo inlink('articleRank', "mode=$mode&state=extra");?>"><?php echo $lang->stat->page->message;?></a>
          </li>
        </ul>
      </div>
    </div>
  ...
  <table class='table tablesorter table-fixed'>
    <thead>
      <tr class='text-center'>
        ...
        <th class='text-left'><?php echo $lang->stat->page->title;?></th>
        <th class='w-300px text-center'><?php echo $lang->stat->page->author;?></th>
        <th class='w-100px text-center'><?php echo $lang->stat->page->{$stateLang};?></th>
        <th class='w-150px text-center'><?php echo $lang->stat->page->pubDate;?></th>
    ...
    <tr>
      ...
      <td class='text-gray text-ellipsis'><?php echo html::a($this->loadModel('article')->createPreviewLink($page->item), $page->title, "target='_blank' class='text-muted'");?></td>
      <td class='text-center'><?php echo $page->author;?></td>
      <td class='text-center'><?php echo $page->{$state};?></td>
      <td class='text-center'><?php echo $page->addedDate;?></td>
      <?php if($mode == 'weekly' || $mode == 'monthly'):?>
      <td class='text-center'><?php echo html::a(inLink('articleTrend', "articleID={$page->item}&mode={$mode}"), $this->lang->stat->domainTrend, "data-toggle='modal' data-type='iframe' class='text-muted'");?></td>
      <?php endif;?>
    </tr>
    <?php endforeach;?>
  </table>
