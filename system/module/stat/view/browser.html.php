  ...
  <header class='clearfix'>
    <ul class='nav nav-tabs-main pull-left'>
      ...
      <li <?php if($mode == $code) {echo "class='active'";}?>>
        <?php echo html::a(inlink('browser', "state=$state&mode=$code"), $modeName);?>
      </li>
      <?php endforeach;?>
    </ul>
    <div class='search pull-left'>
      <form method='get' class='input-control search-box' data-ride='searchbox'>
        ...
        <?php echo html::hidden('f', 'browser');?>
    ...
    <div class="dropdown" id="switchBar"><a href="###" data-toggle="dropdown" class="btn"><?php echo $lang->stat->{$stateLang};?> <span class="icon-angle-down"></span></a>
      <ul class="dropdown-menu">
        <li <?php echo $state == 'pv' ? "class='active'" : '';?> data-type='pv'>
          <a href="<?php echo inlink('browser', "state=pv&mode=$mode&begin=$begin&end=$end");?>"><?php echo $lang->stat->pv;?></a>
        </li>
        <li <?php echo $state == 'uv' ? "class='active'" : '';?> data-type='uv'>
          <a href="<?php echo inlink('browser', "state=uv&mode=$mode&begin=$begin&end=$end");?>"><?php echo $lang->stat->uv;?></a>
        </li>
        <li <?php echo $state == 'ip' ? "class='active'" : '';?> data-type='ip'>
          <a href="<?php echo inlink('browser', "state=ip&mode=$mode&begin=$begin&end=$end");?>"><?php echo $lang->stat->ipCount;?></a>
        </li>
      </ul>
    </div>
  </div>
  <div class='panel-body'>
    <div class='chart-canvas <?php echo isset($totalState) ? "pie" : "center";?>'><canvas height='300' width='900' id='pieChart'></canvas></div>
