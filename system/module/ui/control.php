...
class ui extends control
{
    ...
    public function deleteFavicon()
    {
        ...
        if($favicon)
        {
            $this->loadModel('file')->delete($favicon->fileID);
        }
        else
        {
        ...
        $this->send(array('result' => 'success', 'locate' => inlink('setlogo')));
    }
    ...
    {   
        ...
        foreach($this->lang->site->customizableList as $type => $name)
        {
            if(strpos($this->config->site->modules, $type) !== false)
            {   
                $this->app->loadModuleConfig($type);
    ...
    public function uploadTheme()
    {
        ...
        set_time_limit(0);
        $canManage = array('result' => 'success');
        if(!$this->loadModel('guarder')->verify(true)) $canManage = $this->loadModel('common')->verifyAdmin();
    ...
    public function setDevice($device, $fromVisual = 0)
    {
        if($device == 'mobile')
        {
            if(empty($this->config->site->mobileTemplate))
            {
            ...
            elseif($this->config->site->mobileTemplate == 'close')
            {
                $device = 'desktop';
            }
        }
    ...
    public function themeStore($type = 'byindustry', $param = 'all', $recTotal = 0, $recPerPage = 10, $pageID = 1)
    {
        ...
        $this->view->bindedUser = $this->loadModel('admin')->getUserByApi(true);
     ...
     * @return void
     */
    public function effect($pageID = 1, $recTotal = 0, $recPerPage = 10)
    {
        $recPerPage = $this->cookie->pagerUiEffect ? $this->cookie->pagerUiEffect : $recPerPage;
        ...
        $result = $this->ui->getEffectListByApi(array_keys($this->view->blocks), $pageID, $recTotal, $recPerPage);
