...
class replyModel extends model
{
    ...
    public function getList($orderBy = 'addedDate_desc', $pager = null)
    {
        ...
        $replies = $this->dao->select('t1.*,t2.title as threadTitle,t2.board')->from(TABLE_REPLY)->alias('t1')
            ->leftjoin(TABLE_THREAD)->alias('t2')->on('t1.thread=t2.id')
            ->where(1)
            ->beginIf($searchWord)
            ->andwhere('t1.content')->like("%{$searchWord}%")
            ->fi()
            ->orderBy($orderBy)
            ->page($pager)
            ->fetchAll('id');
        ...
        $boards = $this->dao->select('*')->from(TABLE_CATEGORY)->where('grade')->eq(2)->andWhere('type')->eq('forum')->fetchAll('id');
        foreach($replies as $replie)
        {
            $replie->boardName = $boards[$replie->board]->name;
        }
