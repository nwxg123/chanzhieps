...
class actionModel extends model
{
    ...
    /**
     * Get all actions of an object.
     * 
     * @param  string    $extra  front or admin
     * @param  object    $pager 
     * @access public
     * @return array
     */
    public function getAllList($extra = 'front', $pager = null)
    {
        $actions = $this->dao->setAutoLang(false)->select('*')->from(TABLE_ACTION)
            ->where('extra')->eq($extra)
            ->orderBy('id_desc')
            ->page($pager)
            ->fetchAll();
