...
class book extends control
{
    ...
    { 
        ...
        $book    = $article->book;
        $serials = $this->book->computeSN($book->id);
        $article = $this->book->addMenu($article);
        ...
        $this->view->logo        = $logo;
        $this->view->nodeID      = $article->book->id;
     ...
     * @return void
     */
    public function searchFront($nodeID = '',$words = '', $pageID = 1)
    {
        if(empty($this->config->book->fullScreen) and !$this->get->fullScreen) $this->locate(inlink('index'));
        ...
        $nodeID = $this->get->nodeID ? $this->get->nodeID : $nodeID;
        $words  = $this->get->words ? $this->get->words : $words;
        ...
        $pager->app->control->app->params['nodeID'] = $nodeID;
        $pager->app->control->app->params['words']  = $words;
        ...
        $idList = $this->book->getArticleIdList($nodeID, $families, $allNodes);
        $this->view->results = $this->search->getList($words, $pager, 'book', $idList);
        ...
        $this->view->mobileTitle = $this->lang->book->common;
        $this->view->serials     = $serials;
        $this->view->catalog     = $this->book->getFrontCatalog($book->id, $serials);
        $this->view->allCatalog  = $this->book->getFrontCatalog($book->id, $serials);
        $this->display();
    }
