...
class bookModel extends model
{
        ...
        foreach($nodeList as $parent => $nodes)
        {
            ...
            foreach($nodes as $node)
            {
                ...
                if($node->type == 'chapter')
                {
                    if($this->config->book->chapter == 'left' or $this->config->book->fullScreen or $this->get->fullScreen)
                    {
                        ...
                        { 
                            ...
                            $link =  $node->link;
                            $title =  html::a($link, $node->title, $target);
                        }
                        $catalog .= "<dd class='catalogue chapter text-nowrap text-ellipsis closed' title='{$node->title}'>";
                        $catalog .= "  <span><i class='icon icon-caret-right'></i><span class='order'>$serial.</span>&nbsp;" . $title . "</span>";
                    }
                    else
                    {
                        ...
                        $catalog .= "<dd class='catalogue chapter text-nowrap text-ellipsis closed' title='{$node->title}'>";
                        $catalog .= "  <span><i class='icon icon-caret-right'></i><span class='order'>$serial.</span>&nbsp;" . $node->title . "</span>";
                    }
                }
                elseif($node->type == 'article')
                {
                    ...
                    $catalog .= "<dd id='article{$node->id}' class='catalogue article text-nowrap text-ellipsis' title='{$node->title}'>";
                    $catalog .= " <strong><span class='order'>$serial</span></strong>&nbsp;" . html::a($link, $node->title, $target);
                }
        ...
        foreach($nodeList as $parent => $nodes)
        {
            ...
            foreach($nodes as $node)
            {
                ...
                elseif($node->type == 'article')
                {
                    ...
                    $catalog .= '      <span class="title">' . $serial . ' ' . $node->title . '</span>';
                    $catalog .= '    </span>';
                    $catalog .= $node->grade == 2 ? '</li></a><div class="divider"></div>' : '</li></a>';
                }
    ...
    public function getAdminCatalog($nodeID, $serials)
    {
        ...
        $titleLink   = $node->title;
        $editLink    = commonModel::hasPriv('book', 'edit') ? html::a(helper::createLink('book', 'edit', "nodeID=$node->id"), "<i class='icon icon-edit'></i>", $anchor . "class='btn btn-icon' title='{$this->lang->edit}'") : '';
        $delLink     = commonModel::hasPriv('book', 'edit') ? html::a(helper::createLink('book', 'delete', "bookID=$node->id"), "<i class='icon icon-delete'></i>", "class='btn btn-icon deleter' title='{$this->lang->delete}'") : '';
        $filesLink   = commonModel::hasPriv('file', 'browse') ? html::a(helper::createLink('file', 'browse', "objectType=book&objectID=$node->id&isImage=1"), "<i class='icon icon-image'></i>", "data-toggle='modal' data-width='1000' title='{$this->lang->book->images}' class='btn btn-icon'") : '';
        $filesLink   .= commonModel::hasPriv('file', 'browse') ? html::a(helper::createLink('file', 'browse', "objectType=book&objectID=$node->id&isImage=0"), "<i class='icon icon-paperclip'></i>", "data-toggle='modal' data-width='1000' title='{$this->lang->book->files}' class='btn btn-icon'") : '';
        $catalogLink = commonModel::hasPriv('book', 'catalog') ? html::a(helper::createLink('book', 'catalog', "nodeID=$node->id"), "<i class='icon icon-sitemap'></i>", "class='btn btn-icon' title='{$this->lang->book->catalog}'") : '';
        $moveLink    = commonModel::hasPriv('book', 'sort') ? html::a('javascript:;', "<i class='icon icon-move'></i>", "class='btn btn-icon sort sort-handle'") : '';

        if($node->type == 'article') $previewLink = commonModel::hasPriv('book', 'read') ? html::a($this->createPreviewLink($node->id), "<i class='icon icon-eye'></i>", "target='_blank' class='btn btn-icon' title='{$this->lang->preview}'") : '';
        ...
        if($node->type == 'book')
        {
            $catalog .= "<dt class='book' data-id='" . $node->id . "'>";
            $catalog .= "  <div class='book-title'>";
            $catalog .= "    <strong>" . $titleLink . "</strong>";
            $catalog .= "  </div>";
            $catalog .= "  <span class='actions'>" . $editLink . $catalogLink . $delLink . "</span>";
            $catalog .= "</dt>" . $childrenHtml;
        }

        if($node->type == 'chapter')
        {
            $catalog .= "<dd class='catalog chapter' data-id='" . $node->id . "'>";
            $catalog .= "  <strong>";
            $catalog .= "    <label>" . $this->lang->book->catalog . "</label>";
            $catalog .= "    <div class='title text-ellipsis' title='" . $titleLink . "'>" . $serial . " " . $titleLink . "</div>";
            $catalog .= "  </strong>";
            $catalog .= "  <span class='actions'>" . $editLink . $catalogLink . $delLink . $moveLink . "</span>" . $childrenHtml;
            $catalog .= "</dd>";
        }

        $draft = $node->status == 'draft' ? "<span class='label'>{$this->lang->book->statusList['draft']}</span>" : '';
        if($node->type == 'article')
        {
            $catalog .= "<dd class='catalog' data-id='" . $node->id . "'>";
            $catalog .= "  <strong>";
            $catalog .= "    <label>" . $this->lang->book->articleList . "</label>";
            $catalog .= "    <div class='title text-ellipsis' title='" . $node->title . "'>" . $serial . " " . $node->title . "</div>";
            $catalog .= "  </strong> " . $draft;
            $catalog .= "  <span class='actions'>" . $editLink . $previewLink . $filesLink . $delLink . $moveLink . "</span>" . $childrenHtml;
            $catalog .= "</dd>";
        }
    ...
    public function createBook()
    {
        ...
        $this->dao->update(TABLE_BOOK)
            ...
            ->beginIF($book->order == '0')->set('`order` = ' . $bookID)->fi()
            ->where('id')->eq($bookID)
            ->exec();
    ...
    public function update($nodeID)
    {
        ...
        $node = fixer::input('post')
            ...
            ->add('editedDate',    $now)
            ->setIF($this->post->status == 'draft', 'addedDate', $now)
            ->setForce('keywords', seo::unify($this->post->keywords, ','))
            ->setForce('alias',    seo::unify($this->post->alias, '-', true))
     ...
     * @return string;
     */
    public function addMenu($article)
    {
        $nav = "<ul class='nav nav-content'>";
        $content = str_replace('<h3', '<h4', $article->content);
        ...
        if(count($result[0]) >= 2)
        {
            ...
            $nav .= "</ul>";
            if(empty($this->config->book->fullScreen) and !$this->get->fullScreen) $content = $nav . "<div class='content'>" . $content . '</div>';
        }
