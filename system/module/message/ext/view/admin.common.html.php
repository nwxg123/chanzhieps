<table class='table table-borderless w-p100'>
  ...
  <?php if($original):?>
  <tr class='original'>
    <?php if($original->type != 'message' or ($original->type == 'message' and $original->objectType == 'order')):?>
    <td>
      <?php
      ...
      {
          $objectViewLink = html::a($original->objectViewURL, $original->objectTitle, $original->objectType == 'order' ? "data-toggle='modal'" : "target='_blank'");
      }
      else
      {
      ...
      echo <<<EOT
      <i class='icon-user'></i> <strong>$original->from</strong> &nbsp; <i class='icon-envelope green icon'></i> $original->email &nbsp; 
...
EOT;
  ...
  <tr <?php if($original) echo 'class="separator"';?>>
    ...
    <td>
      ...
      {
          $objectViewLink = $message->objectType != 'order' ? html::a($message->objectViewURL, $message->objectTitle, "target='_blank'") : $message->objectTitle;
      }
      else
      {
          $objectViewLink = "<span class='alert-error'>{$lang->comment->deletedObject}</span>";
