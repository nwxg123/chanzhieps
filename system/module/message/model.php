...
class messageModel extends model
{
     ...
     * @return int
     */
    public function getUncheckedMessages()
    {
        return $this->dao->select('1')->from(TABLE_MESSAGE)->where('status')->eq(0)->count();
    }
     ...
     * @return void
     */
    public function getCountMessage($objectType = '', $status = '')
    {
        return $this->dao->select('objectID, count(1) as count')->from(TABLE_MESSAGE)
    ...
    public function post($type, $block = '')
    {
        ...
        if($message->objectType == 'article')
        {
            $this->loadModel('common')->saveReportRank($message->objectType, $message->objectID, 'extra');
        }
    ...
    public function deleteMessage($messageID, $mode, $maxId = 0)
    {
        $this->dao->delete()
            ...
            ->beginIF($mode == 'batch')->andWhere('id')->in($messageID)->fi()
    ...
    public function pass($messageID, $type, $maxId = 0)
    {
        ...
        else
        {
            ...
            if($message->type == 'reply' and strpos("message,comment", $message->objectType) !== false)
            {
                $this->pass($message->objectID, $type, $maxId);
            }
        }
