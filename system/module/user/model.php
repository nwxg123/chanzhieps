...
class userModel extends model
{
    ...
    public function getList($pager = null, $user = '', $orderBy = 'id_desc')
    {
        ...
        $users  = $this->dao->setAutolang(false)
            ...
            ->beginIf($status && !strpos(',admin,email,all,', $status))->andWhere('o.provider')->eq($status)->fi()
            ->orderBy($orderBy)
            ->page($pager)
            ->fetchAll('id');
    ...
    public function create()
    {
        ...
        $this->dao->insert(TABLE_USER)
            ...
            ->check('email', 'email')
            ->check('email', 'unique')
            ->checkIF($this->post->mobile, 'mobile', 'phone')
    ...
    public function saveLog($account, $result)
    {
        ...
        if(strpos($ip, ',') !== false) list($ip, $ip1) = explode(',', $ip);
        $location = IP::find($ip);
    ...
    {
        return $this->dao->select(1)->from(TABLE_USER)->count();
    }
     ...
     * @return int
     */
    public function getRecentlyAdd($begin = '', $end = '')
    {
        return $this->dao->select(1)->from(TABLE_USER)
            ->where('1=1')
            ->beginIF(!$begin && !$end)->andWhere('`join`')->ge(date('Y-m-d'))
            ->beginIf($begin != '')->andWhere('`join`')->ge($begin)->fi()
            ->beginIf($end != '')->andWhere('`join`')->le($end)->fi()
            ->count();
    }
    
    /**
     * Login user is or not is Wechat User. 
     * 
     * @access public
     * @return bool
     */
    ...
    {
        $user = $this->dao->setAutolang(false)->select('*')->from(TABLE_OAUTH)
            ->where('provider')->eq('wechat')
            ->andWhere('account')->eq($this->app->user->account)->fetch();

        if(!isset($user->provider) || $user->provider != 'wechat') return false;
        return true;
    }
}
