...
<div class='container w-200px'>
  <div class='alert with-icon alert-deny'>
    ...
    <div class='content'>
      ...
      <?php if(!commonModel::isAuthorized($module, $method)):?>
      <p><?php printf($lang->user->errorUnAuthorized, $moduleName, $methodName);?></p>
      <?php else:?>
      ...
      <div class='actions'>
        ...
        if(!commonModel::isAuthorized($module, $method)) echo html::a($this->createLink('admin', 'license'), $lang->user->getAuthorization, "class='btn btn-link'");
        if(commonModel::isAuthorized($module, $method)) echo html::a($this->createLink('user', 'logout', "referer=" . helper::safe64Encode($denyPage)), $lang->user->relogin, "class='btn btn-link'");
        ?>
      </div>
    </div>
  </div>
