...
class threadModel extends model
{
    ...
    public function getList($board, $orderBy, $pager = null, $mode = '') 
    {
        ...
        $threads = $this->dao->select('t1.*,t2.name as boardName')->from(TABLE_THREAD)->alias('t1')
            ->leftjoin(TABLE_CATEGORY)->alias('t2')->on('t1.board=t2.id')
            ->where(1)
            ->beginIf(RUN_MODE == 'front')->andWhere('t1.hidden')->eq('0')->andWhere('t1.addedDate')->le(helper::now())->fi()
            ->beginIf(RUN_MODE == 'front' and $this->config->forum->postReview == 'open')->andWhere('t1.status')->eq('approved')->fi()
            ->beginIf($board)->andWhere('t1.board')->in((array) $board)->fi()
            ->beginIf($mode == 'latest')->andWhere('t1.link')->eq('')->fi()
            ->beginIf($searchWord)
            ->andWhere('t1.title', true)->like("%{$searchWord}%")
            ->orWhere('t1.content')->like("%{$searchWord}%")
            ->markRight(1)
            ->fi()
            ->orderBy($orderBy)
            ->page($pager)
    ...
    /**
     * Get unverify By Account
     *
     * @access public
     * @return int
     */
    public function getUncheckedThreads()
    {
        return $this->dao->select('1')->from(TABLE_THREAD)->where('status')->eq('wait')->count();
    }
    ...
    public function transfer($threadID, $oldBoard, $targetBoard)
    {
        ...
        $oldThread = new stdclass();
        $oldThread->thread = $newThreadID;
        $this->dao->update(TABLE_REPLY)->data($oldThread)->where('thread')->eq($threadID)->exec();
...
EOT;
    ...
    {
        return $this->dao->select(1)->from(TABLE_THREAD)
            ->where('hidden')->eq(0)
            ->count();
    }
}
