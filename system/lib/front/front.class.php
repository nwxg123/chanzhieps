...
class html
{
    ...
    static public function radio($name = '', $options = array(), $checked = '', $attrib = '', $type = 'inline')
    {
        ...
        foreach($options as $key => $value)
        {
            $class = $isBlock ? "block" : "inline-block";
            $string .= "<div class='radio-primary $class'>";
     ...
     * @param  int    $single    is single checkbox.
     * @return string
     */
    static public function checkbox($name, $options, $checked = "", $attrib = "", $type = 'inline', $single = 0)
    {
        $options = (array)($options);
        if(!is_array($options) or empty($options)) return false;
        ...
        foreach($options as $key => $value)
        {
            ...
            else $string .= "<div class='checkbox-primary inline-block'>";
            if($single)  $string .= "<input type='checkbox' id='{$name}' name='{$name}' value='$key' ";
            else $string .= "<input type='checkbox' id='{$name}{$i}' name='{$name}[]' value='$key' ";
            $string .= strpos($checked, ",$key,") !== false ? " checked ='checked'" : "";
            $string .= " $attrib />";
            $string .= "<label>$value</label></div>\n";
...
$(function()
{
    ...
    {
        if($(this).prop("checked") == true) $('.btn-batch').removeAttr('disabled');
    });
    ...
    $(document).on('change', '.check-all', function()
    {
        ...
        scope.find('input:checkbox').each(function() { $(this).prop("checked", e.prop("checked"));});

        $('.btn-batch').attr('disabled','disabled');
        $('tbody input[type=checkbox]').each(function() { if($(this).prop("checked") == true) $('.btn-batch').removeAttr('disabled');});
    });
    ...
    $(document).on('change', 'tbody input[type=checkbox]', function()
    {
        ...
        $('tbody input[type=checkbox]').each(function()
        {
            if($(this).prop("checked") == false) checkAll = false;
        })
...
EOT;
        global $lang;
        $string .= "<div class='checkbox-primary'><input type='checkbox' class='check-all' data-scope='$scope' href='javascript:;' name='checkAll'/><label for='checkAll' class='no-margin'>{$lang->selectAll}</label></div>";
        return  $string;
    }
